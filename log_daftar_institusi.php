...
$limit = 5;
$query = "SELECT * FROM log_daftar_institusi";
...
$pendaftar_institusi = query("SELECT id, ROW_NUMBER() OVER (ORDER BY id ASC) as nomor, waktu, status, id_pendaftar FROM log_daftar_institusi LIMIT $page_awal, $limit");
          ...
          <div class="nav">
            ...
            <div class="collapse" id="collapseInstitusi" aria-labelledby="headingTwo" data-bs-parent="#sidenavAccordion">
              <nav class="sb-sidenav-menu-nested nav">
                ...
                <a class="nav-link active" href="#">Log Daftar Institusi</a>
                          ...
                          <div class="modal-dialog">
                            <div class="modal-content">
                              <div class="modal-header">
                                <h5 class="modal-title text-dark" id="exampleModalLabel">Detail Log No. <?= $institusi['nomor'] ?></h5>
  ...
  <nav aria-label="Page navigation example">
    <ul class="pagination justify-content-center">
      <?php if (empty($_GET['page']) and $pagination == true) : ?>
        <li class="page-item active"><a class="page-link" href="log_daftar_institusi.php?page=1">1</a></li>
        <?php for ($page = 2; $page <= $jumlah_page; $page++) : ?>
          <li class="page-item"><a class="page-link text-warning" href="log_daftar_institusi.php?page=<?= $page ?>"><?= $page ?></a></li>
        <?php endfor; ?>
      <?php elseif (!empty($_GET['page']) and $pagination == true) : ?>
        <?php for ($page = 1; $page <= $jumlah_page; $page++) : ?>
          <?php if ($page == $_GET['page']) : ?>
            <li class="page-item active"><a class="page-link" href="log_daftar_institusi.php?page=<?= $page ?>"><?= $page ?></a></li>
          <?php else : ?>
            <li class="page-item"><a class="page-link text-warning" href="log_daftar_institusi.php?page=<?= $page ?>"><?= $page ?></a></li>
          <?php endif; ?>
        <?php endfor; ?>
