<?php
  ...
  function tambah_individu($post) {
    ...
    try {
      $query_search_procedure = "SELECT 1 FROM mysql.proc p WHERE db = 'psm' AND name = 'tambah_daftar_individu'";
    
      if(mysqli_num_rows(mysqli_query($koneksi, $query_search_procedure)) == 0) {
        $query_add_procedure = "CREATE PROCEDURE tambah_daftar_individu(
        ...
        mysqli_query($koneksi, $query_add_procedure) or die('Procedure Gagal Dibuat');
      }
  ...
  function tambah_institusi($post) {
    ...
    $query_search_procedure = "SELECT 1 FROM mysql.proc p WHERE db = 'psm' AND name = 'tambah_daftar_institusi'";
  
    if(mysqli_num_rows(mysqli_query($koneksi, $query_search_procedure)) == 0) {
      $query_add_procedure = "CREATE PROCEDURE tambah_daftar_institusi(
        IN nama_anggota varchar(50),
        IN jumlah_anggota int(9),
        IN alamat_anggota varchar(100),
        IN no_telp_anggota varchar(50),
        IN cabang_anggota varchar(50)
      )
    ...
    return mysqli_query($koneksi, $query) or die('Tambah Data Dengan Procedure Gagal');;
  }
